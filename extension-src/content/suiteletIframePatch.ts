(function patchSuiteletIframeRequests() {
  ...
  const shouldPatchUrl = (value: unknown) => {
    try {
  ...
  const withEmbedParams = <T>(value: T): T | string => {
    try {
      const url = new URL(String(value), window.location.href);
  ...
  const pendingProxyRequests = new Map<
    string,
    { resolve: (response: any) => void; timeout: ReturnType<typeof setTimeout> }
  >();
  ...
  const proxyRequest = (payload: Record<string, unknown>) =>
  ...
  const canProxyBody = (body: unknown) =>
    body == null ||
    typeof body === "string" ||
    body instanceof URLSearchParams;

  const normalizeProxyBody = (body: any) => {
    if (body instanceof URLSearchParams) return body.toString();
    return body;
  };

  const patchForm = (form: EventTarget | null) => {
  ...
  window.XMLHttpRequest =
    SuiteletProxyXMLHttpRequest as unknown as typeof XMLHttpRequest;
