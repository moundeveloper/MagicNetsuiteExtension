...
interface HelpPageLink {
  text: string;
  url: string;
  section?: string;
}

const normalizeDocWhitespace = (text: string) =>
  text
    .replace(/\r/g, "")
    .replace(/[ \t\f\v]+/g, " ")
    .replace(/ *\n */g, "\n")
...
const toAbsoluteDocUrl = (href: unknown, pageUrl: string) => {
...
const getNearestDocHeading = (element: Element) => {
...
const extractHelpPageContent = (root: Element, pageUrl: string) => {
  const links: HelpPageLink[] = [];
  const seenLinks = new Set<string>();

  const addLink = (anchor: Element, text: string, url: string) => {
    if (!url) return;
  ...
  const serializeChildren = (element: Element): string =>
...
export const setupMessageListener = (bridgeCapability: string) => {
        ...
        return fetch(url, { credentials: "include" })
          ...
          .then(csv => {
            ...
            for (let i = 1; i < lines.length; i++) {
              ...
              const obj: Record<string, string> = { type: labeledType };
