(() => {
  ...
  const createCaptureOverlay = (rect: BoxRect) => {
  ...
  const getParentFrameRect = () =>
    new Promise<FrameRect>((resolve, reject) => {
  ...
  const getViewportRect = async (element: Element) => {
    const rect = element.getBoundingClientRect();
  ...
  const showToast = (message: string, tone = "ok") => {
  ...
  function getElementUnderCursor(x: number, y: number) {
