 ...
 */
const getAllModuleLinks = async (baseUrl: string) => {
  ...
  return Array.from(rows)
    .map((row) => row.querySelector<HTMLAnchorElement>("td p a"))
...
const parseOverviewTable = (table: Element) => {
...
const parseParametersTable = (tableContainer: Element) => {
...
const parseEnumValues = (container: Element) => {
  const values: string[] = [];
...
const parseTable = async (table: Element, pageUrl: string) => {
  ...
  const rowspanMap: Record<number, { value: string; remaining: number }> = {};
...
const fetchModuleData = async (url: string) => {
  const doc = await fetchDoc(url);
  const sections = Array.from(doc.querySelectorAll("h2"));
  const moduleData: Array<{ section: string; table: Array<Record<string, any>> }> = [];
 ...
 */
export const scrapeSuiteScriptModules = async (
  baseUrl: string,
  sendChunk: (chunk: Record<string, unknown>) => void
) => {
  if (!baseUrl) {
