...
export function createServer(): McpServer {
  ...
  async function finishTemplateReviewAction(opts: {
    ...
    try {
      ...
      if (!effectiveRecordId && renderPdf) rendered.renderError = "Select a NetSuite record ID to generate the PDF preview.";
  ...
  server.registerTool("magic_netsuite_template_preview_playwright", {
    ...
    description:
      "Open the professional Reference → HTML → FreeMarker → NetSuite PDF → Final Approval workflow. For recreation from an image, forward the prompt attachment with referenceImagePath or referenceImageDataUrl; missing references are rejected by default instead of silently opening an empty panel.",
    inputSchema: {
  ...
  server.registerTool("magic_netsuite_template_review_update", {
    ...
    description:
      "Live-update the already-open Playwright template review after applying fixes. Keeps the same browser window and refreshes the rendered HTML/reference/FreeMarker pane.",
    inputSchema: {
  ...
  server.registerTool("magic_netsuite_freemarker_preview_playwright", {
    ...
    description:
      "Use only after the user explicitly wants the guarded FreeMarker renderer approval session. For normal 'recreate this NetSuite template' work, call magic_netsuite_recreate_template_workflow first and use magic_netsuite_template_preview_playwright for local visual iteration.",
    inputSchema: {
  ...
  server.registerTool("magic_netsuite_freemarker_convert_approved", {
    ...
    description:
      "POST-APPROVAL ONLY. Convert an already approved FreeMarker preview session into a FreeMarker Advanced PDF template and optionally render it. Never call this while recreating a design, before Playwright screenshot review, or before explicit user approval.",
    inputSchema: {
      sessionId: z.string(),
      renderPdf: z.boolean().optional(),
