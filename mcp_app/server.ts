...
export function createServer(): McpServer {
  ...
  server.registerTool("magic_netsuite_recreate_template_workflow", {
    ...
    return toolResult({
      ...
      currentSession,
      requiredNextAction:
        "Call magic_netsuite_template_session_get_current with references, load the best matching composed skill, then create the initial FreeMarker once or read/patch the existing source incrementally.",
  ...
  server.registerTool("magic_netsuite_template_session_get_current", {
    ...
    description:
      "Load current collaborative metadata, references, record context, render status, revisions, and open fix requests. Full FreeMarker is omitted by default; use the read tool for compact source access.",
    inputSchema: {
  ...
  }, async ({ sessionId, includeReferences = true, includeFreemarker = false }) =>
    callExtensionToolResult("magic_netsuite_template_session_get_current", {
      sessionId,
      includeReferences,
      includeFreemarker,
    }),
  ...
  server.registerTool("magic_netsuite_template_session_update", {
    ...
    description:
      "Create the initial complete FreeMarker document or explicitly replace it. Use read + patch for later revisions so unchanged source is not retransmitted.",
    inputSchema: {
      sessionId: z.string().optional(),
