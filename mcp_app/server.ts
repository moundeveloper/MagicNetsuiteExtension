...
// Archived compatibility surface. The dashboard Template Studio workflow is the
// default and does not expose the old HTML approval / wait-loop tools.
const ENABLE_LEGACY_TEMPLATE_REVIEW =
  process.env.MAGIC_NS_ENABLE_LEGACY_TEMPLATE_REVIEW === "1";
...
const NETSUITE_TEMPLATE_RECREATION_WORKFLOW = `CURRENT TEMPLATE STUDIO WORKFLOW:

1. Load the current dashboard session first:
   - Call magic_netsuite_template_session_get_current.
   - The user creates the brief, adds references, selects NetSuite context, and commits the session from Template Studio.
   - If no current session exists, ask the user to commit one in the dashboard. Do not invent a detached session.

2. Load project knowledge:
   - Call magic_netsuite_search_skills with query "netsuite freemarker advanced pdf bfo template".
   - Load the relevant FreeMarker/BFO skill before writing the template.

3. Design directly in FreeMarker:
   - Produce the complete BFO-compatible <pdf> document; there is no intermediate browser HTML approval stage.
   - Use tables, explicit widths, pt units, XML-safe tags, and NetSuite record bindings.
   - Avoid browser-only CSS such as flex, grid, box-shadow, gradients, and CSS variables.

4. Save, render, and inspect collaboratively:
   - For a session with no source, call magic_netsuite_template_session_update once with the complete freemarker.
   - For every later revision, call magic_netsuite_template_session_read for only the relevant section, then magic_netsuite_template_session_patch with exact oldText/newText edits and the returned sourceVersion.
...
async function callExtensionToolResult(
  name: string,
...
async function loadTemplateDesignSkillPreflight(
  session: unknown,
): Promise<Record<string, unknown> | null> {
  if (!isRecord(session) || Number(session.sourceVersion ?? 0) > 0) return null;
  ...
  try {
    const searchPayload = parseToolJson(
      await callExtensionTool("magic_netsuite_search_skills", {
        query,
        includeDisabled: false,
      }),
    );
    const results =
      isRecord(searchPayload) && Array.isArray(searchPayload.results)
        ? searchPayload.results.filter(isRecord)
        : [];
    const baseline = results.find(
      (result) => String(result.name ?? "") === TEMPLATE_DESIGN_SKILL_NAME,
    );
    ...
    const selected = [specific, baseline ?? results[0]]
      ...
      .filter(
        (result, index, items) =>
          items.findIndex((item) => Number(item.id) === Number(result.id)) === index,
      )
      .slice(0, 2);
    ...
    for (const match of selected) {
      ...
      const payload = parseToolJson(
        await callExtensionTool("magic_netsuite_load_skill", { id }),
      );
...
let extensionTemplateReviewState: ControlResult | null = null;

function extensionReviewResult(result: TextToolResult): ControlResult {
  const parsed = parseToolJson(result);
  if (!isRecord(parsed)) {
    throw new Error("The extension returned an invalid template review state.");
  }
  ...
  return {
    ok: true,
    ...parsed,
    ...(image?.data ? { screenshot: image.data } : {}),
  };
}

async function persistTemplateReviewJournal(state: ControlResult): Promise<void> {
  ...
  const journal = JSON.stringify({
    ...
    status,
    pending: pendingStatuses.has(status),
    version: Number(state.version || 0),
    updatedAt: String(state.updatedAt || new Date().toISOString()),
  }, null, 2);
  const directory = path.dirname(TEMPLATE_REVIEW_STATE_FILE);
  const nextFile = `${TEMPLATE_REVIEW_STATE_FILE}.next`;
  const backupFile = `${TEMPLATE_REVIEW_STATE_FILE}.bak`;
  await fs.mkdir(directory, { recursive: true });
  await fs.writeFile(nextFile, journal, "utf8");
  await fs.rm(backupFile, { force: true }).catch(() => undefined);
  await fs.rename(TEMPLATE_REVIEW_STATE_FILE, backupFile).catch(() => undefined);
  await fs.rename(nextFile, TEMPLATE_REVIEW_STATE_FILE);
}

async function callTemplateReviewSurface(
  operation: "open" | "update" | "get" | "screenshot",
  args: Record<string, unknown> = {},
): Promise<ControlResult> {
  const result = await callExtensionTool(
    `magic_netsuite_template_review_surface_${operation}`,
    args,
  );
  const state = extensionReviewResult(result);
  extensionTemplateReviewState = state;
  await persistTemplateReviewJournal(state);
  return state;
}

const templateReviewController = {
  openTemplateReview: (args: Record<string, unknown>) =>
    callTemplateReviewSurface("open", args),
  updateTemplateReview: (args: Record<string, unknown>) =>
    callTemplateReviewSurface("update", args),
  async templateReviewState(): Promise<ControlResult> {
    try {
      return await callTemplateReviewSurface("get");
    } catch (error) {
      if (extensionTemplateReviewState) return extensionTemplateReviewState;
      throw error;
    }
  },
  async waitTemplateReview(timeoutMs = 900000): Promise<ControlResult> {
    ...
    while (Date.now() < deadline) {
      const state = await callTemplateReviewSurface("get");
      if (actionable.has(String(state.status || ""))) return state;
      await new Promise((resolve) => setTimeout(resolve, 750));
    }
    throw new Error("Timed out waiting for template review action.");
  },
};
...
async function resolveTemplateHtml(html: string | undefined, htmlPath?: string): Promise<string> {
  ...
  if (!value) throw new Error("HTML content or htmlPath is required.");
  return value;
}
...
export function createServer(): McpServer {
  ...
  if (ENABLE_LEGACY_TEMPLATE_REVIEW) server.registerPrompt("open_freemarker_preview", {
  ...
  if (ENABLE_LEGACY_TEMPLATE_REVIEW) registerAppTool(
    server,
  ...
  registerAppTool(
    server,
    "magic_netsuite_freemarker_preview_html",
    {
      title: "FreeMarker HTML Preview",
      description:
        "Create and open a guarded FreeMarker renderer preview. First shows HTML for user approval; conversion tools refuse to run until the preview is approved. If renderer components are missing, ask the user before retrying with deployIfMissing:true.",
      inputSchema: {
    ...
    async ({ html, title = "FreeMarker Preview", recordType = "", recordId = "", deployIfMissing = false }) => {
      ...
      if (structured.needsDeploymentApproval) {
        return toolResult(
          structured,
          "FreeMarker renderer components are not deployed. Ask the user whether to deploy them; if refused, stop.",
        );
      }
      return toolResult(
        { ...structured, mode: "freemarker" },
        "Use the preview below to approve the HTML before FreeMarker conversion.",
      );
    },
  );

  server.registerTool("magic_netsuite_recreate_template_workflow", {
    title: "Start Current Template Studio Session",
    description:
      "Load the direct-FreeMarker Template Studio workflow. The user must first commit a current dashboard session containing the prompt, references, and render context.",
    inputSchema: {
    ...
    let skillMatches: unknown = null;
    let skillPreflight: Record<string, unknown> | null = null;
    ...
    try {
      currentSession = parseToolJson(
        await callExtensionTool("magic_netsuite_template_session_get_current", {
    ...
    return toolResult({
      ...
      skillPreflight,
      requiredNextAction:
        "Call magic_netsuite_template_session_get_current with references. Its first-draft skill preflight is loaded automatically; apply it before creating the initial FreeMarker, or read/patch an existing source incrementally.",
      ...
      screenshotTool: "magic_netsuite_template_session_screenshot",
      deploymentRequiresSeparatePermission: true,
      skillMatches,
    }, "Template Studio workflow loaded. Work directly in the current durable FreeMarker session; no HTML approval or stop-hook loop is required.");
  });

  server.registerTool("magic_netsuite_template_session_list", {
    title: "List Template Studio Sessions",
    description:
      "List the durable dashboard Template Studio sessions and identify which one is current.",
    inputSchema: {},
  }, async () =>
    callExtensionToolResult("magic_netsuite_template_session_list"),
  );

  server.registerTool("magic_netsuite_template_session_get_current", {
    title: "Get Current Template Studio Session",
    description:
      "Load current collaborative metadata, references, record context, render status, revisions, and active fix-request todos. For a new session, this also searches and loads the most relevant local/custom FreeMarker design skills before the first draft. Checked history and full FreeMarker are omitted by default.",
    inputSchema: {
  ...
  server.registerTool("magic_netsuite_template_session_set_current", {
    ...
    description:
      "Select an existing dashboard Template Studio session as current for both Claude and the UI.",
    inputSchema: {
      sessionId: z.string(),
    },
  }, async ({ sessionId }) =>
    callExtensionToolResult("magic_netsuite_template_session_set_current", {
      sessionId,
    }),
  );

  server.registerTool("magic_netsuite_template_session_read", {
    title: "Read FreeMarker Source Slice",
    description:
      "Read only the needed lines from the current FreeMarker source. Returns line numbers and sourceVersion for a subsequent compact patch.",
    inputSchema: {
      sessionId: z.string().optional(),
    ...
    callExtensionToolResult("magic_netsuite_template_session_read", args),
  );

  server.registerTool("magic_netsuite_template_session_patch", {
    title: "Patch Current FreeMarker Source",
    description:
      "Apply small exact-text replacements to the durable FreeMarker source. This is the normal revision tool after initial creation and avoids outputting the complete document again.",
    inputSchema: {
      sessionId: z.string().optional(),
      ...
      addressFeedbackIds: z.array(z.string()).optional(),
      render: z.boolean().optional(),
      screenshot: z.boolean().optional(),
      screenshotPage: z.number().int().positive().optional(),
      screenshotWidth: z.number().int().min(800).max(2400).optional(),
    },
  }, async (args) =>
    callExtensionToolResult("magic_netsuite_template_session_patch", args),
  );

  server.registerTool("magic_netsuite_template_session_update", {
    title: "Update Current FreeMarker Design",
    description:
      "Create the initial complete FreeMarker document or explicitly replace it. Before the initial draft, call template_session_get_current and apply its automatically loaded localSkillPreflight. Use read + patch for later revisions so unchanged source is not retransmitted.",
    inputSchema: {
      sessionId: z.string().optional(),
      ...
      contextMode: z.enum(["freestyle", "transaction", "customrecord"]).optional(),
      recordType: z.string().optional(),
      recordId: z.string().optional(),
    ...
    callExtensionToolResult("magic_netsuite_template_session_update", args),
  );

  server.registerTool("magic_netsuite_template_session_render", {
    title: "Render Current Template Session",
    description:
      "Render the current session's saved FreeMarker through NetSuite and persist the PDF or render error back into the dashboard session.",
    inputSchema: {
      sessionId: z.string().optional(),
      screenshot: z.boolean().optional(),
      screenshotPage: z.number().int().positive().optional(),
    ...
    callExtensionToolResult("magic_netsuite_template_session_render", args),
  );

  server.registerTool("magic_netsuite_template_session_screenshot", {
    title: "Capture Rendered PDF",
    description:
      "Rasterize one complete generated PDF page directly from the PDF bytes. Page selection is explicit and independent of browser zoom or scroll; no browser screenshot, Playwright, or chrome.debugger is used.",
    inputSchema: {
      sessionId: z.string().optional(),
      render: z.boolean().optional(),
      page: z.number().int().positive().optional().describe("One-based page number. Defaults to 1."),
      targetWidth: z.number().int().min(800).max(2400).optional().describe("Raster width in pixels. Defaults to 1600."),
    },
  }, async (args) =>
    callExtensionToolResult("magic_netsuite_template_session_screenshot", args),
  );

  function pdfDataUrlFromRenderResult(value: unknown): { pdfDataUrl: string; renderError: string } {
    if (!isRecord(value)) return { pdfDataUrl: "", renderError: value ? "NetSuite returned an invalid PDF render result." : "" };
    ...
    if (!pdf) return { pdfDataUrl: "", renderError: "NetSuite did not return PDF data." };
  ...
  function appendTemplateArtifactImages(result: CallToolResult, preview: Record<string, unknown>): void {
    const screenshot = typeof preview.screenshot === "string" ? preview.screenshot : "";
    ...
    if (screenshot) result.content.push({ type: "image", data: screenshot, mimeType: "image/jpeg" });
    if (htmlScreenshot) result.content.push({ type: "image", data: htmlScreenshot, mimeType: "image/jpeg" });
    if (pdfScreenshot) result.content.push({ type: "image", data: pdfScreenshot, mimeType: "image/jpeg" });
  }

  async function finishTemplateReviewAction(opts: {
    timeoutMs?: number;
    convertOnApprove?: boolean;
    ...
    const review = await templateReviewController.waitTemplateReview(timeoutMs);
    ...
    await templateReviewController.updateTemplateReview({ status: "converting", renderError: "", feedback: "" });
    try {
      ...
      if (previewStructured.needsDeploymentApproval) {
        ...
        await templateReviewController.updateTemplateReview({ renderError: message, status: "render_error" });
        return toolResult(previewStructured, message);
      }

      const sessionId = String(previewStructured.sessionId ?? "");
      if (!sessionId) throw new Error("FreeMarker preview session did not return a sessionId.");
      await templateReviewController.updateTemplateReview({
        sessionId,
        recordType: effectiveRecordType,
        recordId: effectiveRecordId,
        status: "converting",
      ...
      await callExtensionTool("netsuite_freemarker_convert_approved", {
        sessionId,
      ...
      if (!effectiveRecordId && renderPdf) {
        await templateReviewController.updateTemplateReview({
          renderError: "Select a NetSuite record ID to generate the PDF preview.",
      ...
      await templateReviewController.updateTemplateReview({ status: "render_error", renderError: message });
      throw error;
    }
  }
  ...
  function templateReviewOptionContext(recordType = "", recordId = ""): {
    ...
    if (recordType) recordTypes.add(recordType);
    if (recordId) recordIds.add(recordId);
    for (const item of selectedContext?.selectedItems ?? []) {
      if (item.recordType) recordTypes.add(item.recordType);
      if (item.id) recordIds.add(item.id);
    }
    if (!recordTypes.size) recordTypes.add("invoice");
    return {
  ...
  async function loadTemplateReviewRecordOptions(recordType = "invoice"): Promise<{
    recordTypeOptions: string[];
    recordIdOptions: string[];
    ...
    const recordIdLabels: Record<string, string> = {};
    try {
    ...
    for (const sql of recordSearchQueries(recordType || "invoice", "", 100)) {
      try {
        ...
        if (rows.length || sql.includes("SELECT id FROM")) break;
      } catch (error) {
  ...
  const templateReviewOpenSchema = {
  ...
  const openTemplateReviewTool = async ({
    html,
    htmlPath = "",
    title = "NetSuite Template Preview",
    ...
    if (selectedRecordId && !liveOptions.recordIdOptions.includes(selectedRecordId)) liveOptions.recordIdOptions.unshift(selectedRecordId);
    const preview = await templateReviewController.openTemplateReview({
      html: resolvedHtml,
      title,
      templateFile,
    ...
    const result = toolResult(
      ...
      "Template review opened in the Magic NetSuite browser tab. Call magic_netsuite_template_review_wait next; it will return when the user clicks Approve or Send Fixes.",
    );
    appendTemplateArtifactImages(result, preview);
    return result;
  };

  if (ENABLE_LEGACY_TEMPLATE_REVIEW) {
  server.registerTool("magic_netsuite_template_review_open", {
    title: "Open NetSuite Template Review",
    description:
      "Open the extension-owned browser-tab workflow for Reference → HTML → FreeMarker → NetSuite PDF → Final Approval. For recreation from an image, forward the prompt attachment with referenceImagePath or referenceImageDataUrl.",
    inputSchema: templateReviewOpenSchema,
  }, openTemplateReviewTool);

  server.registerTool("magic_netsuite_template_preview_playwright", {
    title: "Open NetSuite Template Review (Legacy Alias)",
    description:
      "Deprecated compatibility alias for magic_netsuite_template_review_open. Despite the old name, this opens the extension browser tab and never starts Playwright.",
    inputSchema: templateReviewOpenSchema,
  }, openTemplateReviewTool);

  server.registerTool("magic_netsuite_template_review_update", {
    title: "Update Browser Template Review",
    description:
      "Live-update the extension-owned template review tab after applying fixes. Keeps durable review state and refreshes the rendered HTML/reference/FreeMarker pane.",
    inputSchema: {
    ...
    const current = await templateReviewController.templateReviewState();
    ...
    let nextStatus: "html_review" | "freemarker_review" | undefined = resolvedHtml ? "html_review" : undefined;

    if (args.rerenderFreemarker) {
      freemarker = freemarker || (typeof current.freemarker === "string" ? current.freemarker : "");
      if (!freemarker.trim()) throw new Error("freemarker is required for NetSuite rerendering.");
      if (!sessionId) throw new Error("The approved FreeMarker session could not be recovered; approve the HTML stage again.");
      ...
      if (!effectiveRecordType || !effectiveRecordId) throw new Error("recordType and recordId are required for NetSuite PDF rendering.");
      try {
        await callExtensionTool("netsuite_freemarker_convert_approved", {
          sessionId,
          freemarker,
        ...
        const message = error instanceof Error ? error.message : String(error);
        await templateReviewController.updateTemplateReview({ freemarker, status: "render_error", renderError: message });
        throw error;
      }
    ...
    const currentComments = (Array.isArray(current.comments) ? current.comments : []).map((comment, index) => {
      if (!isRecord(comment) || typeof comment.id === "string") return comment;
    ...
    const parentComment = currentComments.find((comment) =>
      isRecord(comment) && comment.isYou === true && typeof comment.id === "string" &&
    ...
    const preview = await templateReviewController.updateTemplateReview({
      html: resolvedHtml,
      title: args.title,
      ...
      freemarker: args.rerenderFreemarker ? undefined : freemarker,
      pdfDataUrl,
      renderError,
    ...
    const result = toolResult(
      ...
      "Template review updated in the existing Magic NetSuite browser tab.",
    );
    appendTemplateArtifactImages(result, preview);
    return result;
  });
  ...
  server.registerTool("magic_netsuite_freemarker_preview_playwright", {
    title: "Open Guarded FreeMarker Review",
    description:
      "Legacy compatibility entry point for the guarded FreeMarker approval session. It opens the extension-owned browser review tab and does not start Playwright. For normal template recreation, use magic_netsuite_template_review_open.",
    inputSchema: {
    ...
    const liveOptions = await loadTemplateReviewRecordOptions(recordType);
    const preview = await templateReviewController.openTemplateReview({
      html: previewHtml,
      title,
      recordType: recordType || "invoice",
      recordId,
      recordTypeOptions: liveOptions.recordTypeOptions,
      recordIdOptions: liveOptions.recordIdOptions,
      recordTypeLabels: liveOptions.recordTypeLabels,
      recordIdLabels: liveOptions.recordIdLabels,
      sessionId: String(structured.sessionId || ""),
    });
    const result = toolResult(
      {
        ...structured,
      ...
      "FreeMarker HTML preview opened in the extension browser tab. Inspect the screenshot, then wait for explicit user approval before converting.",
    );
  ...
  server.registerTool("magic_netsuite_freemarker_convert_approved", {
    ...
    description:
      "POST-APPROVAL ONLY. Convert an already approved FreeMarker preview session into a FreeMarker Advanced PDF template and optionally render it. Never call this while recreating a design, before browser-tab screenshot review, or before explicit user approval.",
    inputSchema: {
      sessionId: z.string(),
      renderPdf: z.boolean().optional(),
    ...
    if (!isRecord(data)) return toolResult({ value: data }, "Approved FreeMarker preview converted.");
  ...
  server.registerTool("magic_netsuite_save_skill", {
    ...
    inputSchema: {
      ...
      dependencies: z.array(z.number().int().positive()).optional().describe("Ordered IDs of reusable sub-skills loaded with this skill."),
  ...
  server.registerTool("magic_netsuite_load_skill", {
    title: "Load Composed Magic NetSuite Skill",
    description: "Load one skill by ID together with its recursively composed reusable sub-skills.",
    inputSchema: {
      id: z.number().optional(),
  ...
  server.registerTool("magic_netsuite_suitelet_control_open", {
    ...
    if (deployId) openArgs.deployId = deployId;
    if (url) openArgs.url = url;
