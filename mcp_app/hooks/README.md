...
When the review status is pending (`open`, `needs_changes`, `approved`, or `ftl_review`), the hook returns:

```json
{
  "decision": "block",
  "reason": "The NetSuite template review is still pending in Playwright. Call magic_netsuite_template_review_wait and continue after the user clicks Send Fixes, Approve, or End."
}
```

It allows stopping when no review is open, when the user clicks `End`, or when Claude Code reports `stop_hook_active`.
