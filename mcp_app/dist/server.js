  ...
  const connecting = new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        ...
        reject(
          new Error(
            "Open the Magic NetSuite extension MCP Server page and enable the bridge."
          )
        );
      }
    }, 2500);
    ...
    socket.once("error", (err) => {
      ...
      if (!settled) {
        ...
        reject(
          new Error(
            `Magic NetSuite native bridge is not connected: ${err.message}`
          )
        );
      }
    });
...
function recordSearchQueries(recordType, query, limit) {
  ...
  if (TRANSACTION_TYPES[cleanType]) {
    const conditions = [
      `type = '${TRANSACTION_TYPES[cleanType]}'`,
      `ROWNUM <= ${rowLimit}`
    ];
    if (cleanQuery) {
...
function mapFolderRow(row) {
  return {
    ...
    lastmodifieddate:
      row.lastmodifieddate == null ? null : String(row.lastmodifieddate),
...
async function resolveSuiteletUrl(args) {
  ...
  if (args.scriptId && args.deployId && args.origin) {
    ...
    return {
      url: `${args.origin}/app/site/hosting/scriptlet.nl?script=${sid}&deploy=${did}`,
      name: ""
    };
  }
  const query = args.query || args.scriptId || "";
  const data = parseToolJson(
    await callExtensionTool("netsuite_suitelet_stream_list", { query })
  );
  ...
  const match =
    suitelets.find((s) => {
      if (
        args.scriptId &&
        String(s.scriptId).toLowerCase() === args.scriptId.toLowerCase()
      ) {
        return (
          !args.deployId ||
          String(s.deploymentId).toLowerCase() === args.deployId.toLowerCase()
        );
      }
      return false;
    }) ||
...
async function getNetsuiteCookies() {
  try {
    const data = parseToolJson(
      await callExtensionTool("netsuite_dump_cookies")
    );
    const origin =
      isRecord(data) && typeof data.origin === "string" ? data.origin : "";
    const raw =
      isRecord(data) && Array.isArray(data.cookies) ? data.cookies : [];
    const mapped = [];
    for (const c of raw) {
      ...
      const sameSite =
        ...
        sameSiteRaw === "None"
          ...
          : undefined;
      const expires =
        typeof c.expires === "number" && c.expires > 0 ? c.expires : undefined;
      mapped.push({
        name: String(c.name),
...
export function createServer() {
  ...
  server.registerPrompt(
    "open_context_picker",
    {
      ...
      description:
        "Open the Magic NetSuite context picker MCP App for selecting records and File Cabinet files.",
      argsSchema: {
  ...
  server.registerPrompt(
    "open_suitelet_viewer",
    {
      title: "Open Magic NetSuite Suitelet Viewer",
      description:
        "Open the Magic NetSuite MCP App Suitelet viewer for interactive Suitelet streaming.",
      argsSchema: {
  ...
  registerAppTool(
    ...
    "magic_netsuite_context_picker",
    {
      ...
      description:
        "Open an interactive picker for NetSuite records and File Cabinet files, then load the selected context into Claude chat.",
      inputSchema: {
        initialTab: z
          .enum(["records", "files"])
          .optional()
          .describe("Which picker tab to open first.")
      },
    ...
    async ({ initialTab = "records" }) =>
      toolResult(
        { initialTab },
        "Use the picker below to select NetSuite context for Claude."
      )
  );
  registerAppTool(
    server,
    "magic_netsuite_suitelet_viewer",
    {
      ...
      description:
        "Open an interactive MCP App viewer that streams a NetSuite Suitelet tab and forwards clicks, wheel events, and keyboard input back to Chrome.",
      inputSchema: {
        url: z
          .string()
          .optional()
          .describe(
            "Optional Suitelet URL to open. If omitted, the viewer uses the preferred/current NetSuite tab."
          )
      },
      _meta: { ui: { resourceUri } }
    },
    async ({ url = "" }) =>
      toolResult(
        { mode: "suitelet", url },
        "Use the viewer below to stream and interact with a NetSuite Suitelet."
      )
  );
  server.registerTool(
    "magic_netsuite_bridge_status",
    {
      title: "Magic NetSuite Bridge Status",
      description:
        "Check whether the Magic NetSuite extension native bridge is reachable.",
      inputSchema: {}
    },
    async () => {
      await connectNativeBridge();
      return toolResult(
        { connected: true, pipe: BRIDGE_PIPE_PATH },
        "Magic NetSuite bridge is connected."
      );
    }
  );
  server.registerTool(
    "magic_netsuite_suitelet_stream_start",
    {
      title: "Start Suitelet Stream",
      description:
        "INTERNAL viewer transport for the MCP App UI — agents should NOT use this. To open and drive a Suitelet, use magic_netsuite_suitelet_control_open instead.",
      inputSchema: {
        url: z.string().optional()
      }
    },
    async ({ url = "" }) => {
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_stream_start", { url })
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet stream started."
      );
    }
  );
  server.registerTool(
    "magic_netsuite_suitelet_stream_list",
    {
      title: "List Suitelets",
      description:
        "List deployed Suitelets available for the Magic NetSuite Suitelet Viewer MCP App.",
      inputSchema: {
  ...
  server.registerTool(
    "magic_netsuite_suitelet_stream_frame",
    {
      title: "Get Suitelet Stream Frame",
      description:
        "INTERNAL viewer transport for the MCP App UI — agents should NOT use this. To see the controlled Suitelet, use magic_netsuite_suitelet_screenshot (or scroll/hover, which also return a screenshot).",
      inputSchema: {}
    },
    async () => {
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_stream_frame")
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet frame captured."
      );
    }
  );
  server.registerTool(
    "magic_netsuite_suitelet_stream_input",
    {
      title: "Send Suitelet Stream Input",
      description:
        "INTERNAL viewer transport for the MCP App UI (forwards raw coordinate mouse/wheel/key events) — agents should NOT use this; coordinate input is unreliable. Use the dedicated tools instead: magic_netsuite_suitelet_scroll, _hover, _click, _fill.",
      inputSchema: {
    ...
    async ({ event }) => {
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_stream_input", { event })
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet input sent."
  ...
  server.registerTool(
    "magic_netsuite_suitelet_probe_url",
    {
      title: "Probe Suitelet URL",
      description:
        "Fetch a Suitelet URL from the Chrome extension and return diagnostics for blank iframe troubleshooting.",
      inputSchema: {
        url: z.string()
      }
    },
    async ({ url }) => {
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_probe_url", { url })
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet URL probed."
  ...
  server.registerTool(
    "magic_netsuite_suitelet_fetch_html",
    {
      title: "Fetch Suitelet HTML",
      description:
        "Fetch Suitelet HTML through the Chrome extension for srcdoc rendering in the MCP App.",
      inputSchema: {
        url: z.string()
      }
    },
    async ({ url }) => {
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_fetch_html", { url })
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet HTML fetched."
      );
    }
  );
  server.registerTool(
    "magic_netsuite_suitelet_proxy_request",
    {
      title: "Proxy Suitelet Request",
      description:
        "Proxy a runtime Suitelet fetch/XHR request through the Chrome extension.",
      inputSchema: {
        url: z.string(),
    ...
    async ({
      ...
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_proxy_request", {
          url,
          originalUrl,
          source,
          method,
  ...
  server.registerTool(
    ...
    async ({ query, scriptId, deployId, url }) => {
      if (USE_PLAYWRIGHT) {
        ...
        const resolved = await resolveSuiteletUrl({
          query,
          scriptId,
          deployId,
          url,
        ...
        const data = await playwrightController.open(
          resolved.url,
          cookies,
          resolved.name
        );
        return controlToolResult(
          data,
          "Suitelet opened in Playwright for control."
        );
      }
      const openArgs = {};
      ...
      const raw = await callExtensionTool(
        "netsuite_suitelet_control_open",
        openArgs
      );
      const data = parseToolJson(raw);
      const result = toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet opened for control."
      );
  ...
  server.registerTool(
    "magic_netsuite_suitelet_inspect",
    {
      title: "Inspect Suitelet",
      description:
        "List visible interactive elements (inputs, textareas, selects, buttons, links) of the controlled Suitelet with CSS selectors and labels.",
      inputSchema: {}
    },
    async () => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.inspect(),
          "Suitelet inspected."
        );
      }
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_inspect")
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet inspected."
  ...
  server.registerTool(
    "magic_netsuite_suitelet_screenshot",
    {
      title: "Screenshot Suitelet",
      description:
        "Capture a screenshot of the controlled Suitelet tab so Claude can see it. Use this instead of any external/Claude-in-Chrome browser screenshot tool.",
      inputSchema: {}
    },
    async () => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.screenshot(),
          "Suitelet screenshot captured."
        );
      }
  ...
  server.registerTool(
    "magic_netsuite_suitelet_hover",
    {
      title: "Hover in Suitelet",
      description:
        "Hover the mouse over an element (by CSS selector or x/y viewport coordinates) and return a screenshot. Triggers native :hover plus pointer/mouseover events so tooltips and hover menus appear.",
      inputSchema: {
    ...
    async ({ selector, x, y }) => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.hover({ selector, x, y }),
          "Suitelet hovered."
        );
      }
      const raw = await callExtensionTool("netsuite_suitelet_hover", {
        selector,
        x,
        y
      });
  ...
  server.registerTool(
    "magic_netsuite_suitelet_scroll",
    {
      title: "Scroll Suitelet",
      description:
        'Scroll the controlled Suitelet (window or a scrollable element by selector) and return a screenshot of the new view. Use to reveal a results table below the fold. Omit args to page down; or pass to:"bottom"/"top", a dy delta, or a selector.',
      inputSchema: {
        selector: z.string().optional(),
    ...
    async ({ selector, to, dy, dx }) => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.scroll({ selector, to, dy, dx }),
          "Suitelet scrolled."
        );
      }
      const raw = await callExtensionTool("netsuite_suitelet_scroll", {
        selector,
        to,
        dy,
        dx
  ...
  server.registerTool(
    "magic_netsuite_suitelet_fill",
    {
      title: "Fill Suitelet Field",
      description:
        "Set the value of an input/textarea/select in the controlled Suitelet by CSS selector (fires input/change events).",
      inputSchema: {
        selector: z.string(),
    ...
    async ({ selector, value = "" }) => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.fill({ selector, value }),
          "Suitelet field filled."
        );
      }
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_fill", { selector, value })
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet field filled."
  ...
  server.registerTool(
    "magic_netsuite_suitelet_select",
    {
    ...
    async ({ selector, fieldId, value, label }) => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.select({
            selector,
            fieldId,
            value,
            label
          }),
          "Suitelet option selected."
        );
      }
      const expr = JSON.stringify({ selector, fieldId, value, label });
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_eval", {
  ...
  server.registerTool(
    "magic_netsuite_suitelet_click",
    {
      title: "Click Suitelet Element",
      description:
        'Click an element in the controlled Suitelet by CSS selector or by visible button/link text (e.g. "Run Query").',
      inputSchema: {
        selector: z.string().optional(),
    ...
    async ({ selector, text }) => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.click({ selector, text }),
          "Suitelet element clicked."
        );
      }
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_click", { selector, text })
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet element clicked."
  ...
  server.registerTool(
    "magic_netsuite_suitelet_read",
    {
      title: "Read Suitelet Content",
      description:
        "Read text/value from the controlled Suitelet by CSS selector, or the whole page body when no selector is given.",
      inputSchema: {
        selector: z.string().optional(),
    ...
    async ({ selector, maxLength }) => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.read({ selector, maxLength }),
          "Suitelet content read."
        );
      }
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_read", {
          selector,
          maxLength
        })
      );
  ...
  server.registerTool(
    "magic_netsuite_suitelet_eval",
    {
      title: "Eval in Suitelet",
      description:
        "Run a JavaScript expression/snippet in the controlled Suitelet tab and return the result. Flexible control primitive; prefer fill/click/read for common actions.",
      inputSchema: {
        expression: z.string()
      }
    },
    async ({ expression }) => {
      if (USE_PLAYWRIGHT) {
        return controlToolResult(
          await playwrightController.eval(expression),
          "Suitelet eval complete."
        );
      }
      const data = parseToolJson(
        await callExtensionTool("netsuite_suitelet_eval", { expression })
      );
      return toolResult(
        isRecord(data) ? data : { value: data },
        "Suitelet eval complete."
  ...
  server.registerTool(
    "magic_netsuite_save_selected_context",
    {
      title: "Save Selected NetSuite Context",
      description:
        "Save the context selected in the interactive picker so Claude can retrieve it with magic_netsuite_get_selected_context.",
      inputSchema: {
        markdown: z.string(),
  ...
  server.registerTool(
    "magic_netsuite_get_selected_context",
    {
      title: "Get Selected NetSuite Context",
      description:
        "Retrieve the latest NetSuite records/files selected in the interactive picker. Use this when the user asks about a file or record they selected in the picker.",
      inputSchema: {}
    },
    async () => {
      if (!selectedContext) {
  ...
  server.registerTool(
    "magic_netsuite_list_record_types",
    {
    ...
    async () => {
      const data = parseToolJson(
        await callExtensionTool("netsuite_list_record_types")
      );
      ...
      return toolResult(
        { recordTypes },
        `Found ${recordTypes.length} record types.`
      );
    }
  );
  server.registerTool(
    "magic_netsuite_search_records",
    {
  ...
  server.registerTool(
    "magic_netsuite_load_record_context",
    {
    ...
    async ({ recordType, recordId, includeSublists = false }) => {
      const body = parseToolJson(
        await callExtensionTool("netsuite_load_record", {
          recordType,
          recordId
        })
      );
      ...
      if (includeSublists) {
        sublists = parseToolJson(
          await callExtensionTool("netsuite_get_record_sublists", {
            recordType,
            recordId
          })
        );
      ...
      return markdownToolResult(
        { markdown, body, sublists },
        `Loaded ${recordType} #${recordId}.`
      );
    }
  );
  server.registerTool(
    "magic_netsuite_list_file_cabinet_folder",
    {
      title: "List NetSuite File Cabinet Folder",
      description:
        "List File Cabinet folders and files for the context picker. Pass no folderId for root folders.",
      inputSchema: {
  ...
  server.registerTool(
    "magic_netsuite_search_files",
    {
      title: "Search NetSuite File Cabinet",
      description:
        "Search the NetSuite File Cabinet by file/folder ID or name.",
      inputSchema: {
        query: z.string(),
        limit: z.number().int().min(1).max(100).optional()
      }
    ...
    async ({ query, limit = 50 }) => {
      ...
      const [folderRows, fileData] = await Promise.all([
        ...
        callExtensionTool(
          "netsuite_find_file",
          numeric ? { id: cleanQuery } : { name: cleanQuery }
        ).then(parseToolJson)
      ]);
      ...
      return toolResult(
        { folders, files, breadcrumbs: [], folderId: null },
        `Found ${folders.length} folders and ${files.length} files.`
      );
    }
  );
  server.registerTool(
    "magic_netsuite_read_file_context",
    {
      title: "Read NetSuite File Context",
      description:
        "Read a NetSuite File Cabinet file as text context for Claude.",
      inputSchema: {
        fileId: z.string()
      }
    },
    async ({ fileId }) => {
      const file = parseToolJson(
        await callExtensionTool("netsuite_read_file", { fileId })
      );
      const markdown = [
