...
export const tools: ToolDefinition[] = [
  ...
  // ========== Code Generation ==========
  {
    name: "generate_suitelet_code",
    description:
      "Generate SuiteScript 2.1 Suitelet source code based on a name and description. " +
      "Searches the local skills library for relevant Suitelet/serverWidget patterns and " +
      "applies them to produce a best-practice scaffold. Returns the generated source code " +
      "and the derived file name and script ID. " +
      "This tool is used internally by the generate_suitelet pipeline — prefer calling " +
      "generate_suitelet directly instead of this tool in isolation.",
    parameters: {
      type: "object",
      properties: {
        name: {
          type: "string",
          description: "Human-readable name of the Suitelet (e.g. 'Customer Dashboard')"
        },
        description: {
          type: "string",
          description: "What the Suitelet should do — its purpose, data, actions"
        }
      },
      required: ["name", "description"]
    },
    execute: async (input) => {
      const rawName = String(input.name || "NewSuitelet");
      const description = String(input.description || "");

      const scriptId = `customscript_${rawName
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/(^_|_$)/g, "")}`;
      const fileName = `${scriptId}.js`;

      // Search local skills for relevant Suitelet/serverWidget patterns
      const skillResults = await searchSkills("suitelet serverWidget N/ui/serverWidget");
      const skillNames: string[] = [];
      const skillContents: string[] = [];
      for (const result of skillResults.slice(0, 3)) {
        const full = await getSkillContent(result.id);
        if (full) {
          skillNames.push(full.name);
          skillContents.push(full.content);
        }
      }

      // Build skill guidance block (inlined into comments so no runtime dependency)
      const skillNote = skillNames.length > 0
        ? `// Skills applied: ${skillNames.join(", ")}\n`
        : "";

      // Extract key patterns from skill content (first 500 chars each, as guidance comments)
      const skillGuidance = skillContents.length > 0
        ? skillContents
            .map((content, idx) => {
              const trimmed = content.slice(0, 500).replace(/\*\//g, "* /");
              return `/*\n * Skill guidance [${skillNames[idx]}]:\n * ${trimmed.split("\n").join("\n * ")}\n */`;
            })
            .join("\n")
        : "";

      const code = buildSuiteletScaffold({ rawName, scriptId, description, skillNote, skillGuidance });
...
// ─────────────────────────────────────────────
// Suitelet scaffold builder
// ─────────────────────────────────────────────

function buildSuiteletScaffold({
  rawName,
  scriptId,
  description,
  skillNote,
  ...
  const guidanceBlock = skillGuidance ? `\n${skillGuidance}\n` : "";
  return `/**
 * @NApiVersion 2.1
 ...
 *
 * Script ID : ${scriptId}
 * Deployment: customdeploy_${scriptId.replace(/^customscript_/, "")}
 */
${guidanceBlock}define(['N/ui/serverWidget', 'N/log'], (serverWidget, log) => {

  /**
   * Suitelet entry point — handles GET (render form) and POST (process submission).
  ...
  /**
   * Builds the Suitelet form for GET requests.
   * TODO: customise fields, sublists, and buttons to match requirements.
  ...
  const renderForm = () => {
    ...
    form.addField({
      ...
      label: 'Info'
    }).defaultValue = '<p>Suitelet scaffold — customise this form.</p>';
  ...
  const handlePost = (request, response) => {
    try {
      log.audit('${rawName} POST', 'Form submitted');

      // ── TODO: process submitted data ────────────────────────────────────

      response.write({
        output: '<html><body><h2>Submitted successfully.</h2></body></html>'
      });
    } catch (error) {
      log.error('${rawName} POST Error', error.message);
      response.write({
        output: \`<html><body><h2>Error: \${error.message}</h2></body></html>\`
      });
    }
  };
