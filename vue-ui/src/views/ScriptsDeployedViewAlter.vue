...
import { dummyEditors } from "../utils/tempDummyEditors";
import {
  useCodeViewerSearch,
  type CodeViewerAPI
...
const currentScriptName = computed(() => {
  const activeId = activeViewerId.value; // This will trigger reactivity
  if (!activeId) return "";

  const match = editors.find((item) => item.editor?.id === activeId);
