(() => {
  ...
  const normalizeWithPositions = (root) => {
  ...
  const isolateHelpContent = (helpRoot) => {
  ...
  const disableHelpLinks = (helpRoot) => {
  ...
  const highlightQuote = (helpRoot, quote) => {
    ...
    if (globalThis.CSS?.highlights && typeof globalThis.Highlight === "function") {
      CSS.highlights.set("magic-netsuite-citation", new Highlight(range));
      return {
        range,
  ...
  const scrollHighlightIntoView = ({ range, target }) => {
